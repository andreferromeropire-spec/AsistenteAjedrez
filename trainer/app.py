# -*- coding: utf-8 -*-
"""Flask app para el entrenador táctico de ajedrez."""
...
def session_start():
    ...
    try:
        session_id = database.start_session(conn, 'quick_training', '800-1400', len(filtered))
    finally:
        conn.close()
...
@app.route("/api/puzzle/<int:index>")
...
def session_summary(session_id):
    conn = database.get_connection()
    try:
        stats = database.get_session_stats(conn, session_id)
        insights = statistics.get_scan_insights(conn, session_id)
    finally:
        conn.close()
    return jsonify({
        'total': stats['total'],
...
def api_result():
    ...
    false_positives = []
    for sq_name in marked_set:
        if sq_name not in correct_set:
            sq = chess.parse_square(sq_name)
            piece = board.piece_at(sq)
            if piece is not None:
                defenders = board.attackers(piece.color, sq)
                def_sq = next(iter(defenders), None)
                def_piece = board.piece_at(def_sq) if def_sq is not None else None
                false_positives.append({
                    'square': sq_name,
                    'defender_square': chess.square_name(def_sq) if def_sq is not None else None,
                    'defender_piece': _pieza_es(def_piece) if def_piece is not None else None
                })
            else:
                false_positives.append({'square': sq_name, 'defender_square': None, 'defender_piece': None})

    correct = len(missed) == 0 and len(false_positives) == 0
    puzzle_id = str(p.get('PuzzleId', ''))
    theme = (p.get('Themes') or 'unknown').split()[0] if p.get('Themes') else 'unknown'
    puzzle_rating = int(p.get('Rating', 0))

    conn = database.get_connection()
    try:
        database.save_result(
            conn, session_id, puzzle_id, theme, puzzle_rating,
            correct, elapsed_ms,
    ...
    explanation_parts = []
    if missed:
        explanation_parts.append('Te faltaron marcar: ' + ', '.join(m['square'] + ' (' + m['type'] + ')' for m in missed))
    if false_positives:
        explanation_parts.append('Marcaste de más: ' + ', '.join(fp['square'] for fp in false_positives))
    explanation = ' '.join(explanation_parts) if explanation_parts else 'Correcto.'
